...
//
// backend/ 側にも同名の生成物がありますが、共有せずそれぞれ生成しています
// (同じスキーマから生成するため内容は一致します。docs/adr/0002-shared-types-deferred.md)。
//
// 生成元: Supabase project `OpenOwl-feature` (vaaaiatfouoaljsitstv)、2026-08-18。
//
// 1点だけ手動補正している: record_synonym_generation の Args のうち
// p_prompt_tokens / p_completion_tokens。Postgres の関数引数は NOT NULL 制約がない限り
// 呼び出し時に null を渡せるが、Supabase の型生成は plpgsql 関数引数の nullability を
// 常に non-null として出力するため、生成結果のままだと呼び出し側が型エラーになる。
// 次回再生成時もこの2箇所は同様に `| null` を補うこと(backend 側と同じ補正)。
