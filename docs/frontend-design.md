...
### 13.1 環境変数が埋め込まれていないときの挙動

`EXPO_PUBLIC_*` は**ビルド時**にバンドルへ埋め込まれる。
デプロイ先の環境変数が未設定のままビルドすると、実行時にはもう手当てできない。

当初の実装は `env.ts` のモジュール読み込み時に例外を投げていた。
これは `app/_layout.tsx` の import 連鎖の先で評価されるため、
**React が描画に入る前にバンドルが停止し、画面が真っ白になる**。
本番でこれが起き、DevTools を開くまで原因が分からなかった。

したがって `env.ts` は**読み込み時に throw しない**:

- 検証に失敗しても、実在しないドメイン（`https://invalid.example`）の
  フォールバック値で読み込みを通す。設定不備のまま実通信してしまうことはない
- 失敗の内容は `envError` として export する
- `_layout.tsx` は `envError` があれば `ConfigErrorScreen` を描画する

画面には**変数名だけ**を出し、値は出さない（公開前提の値だが、
出す必要がないものは出さない）。この画面は利用者向けではなく、デプロイした本人向けである。

**ビルドキャッシュに注意**: Expo / Metro は変換結果をキャッシュするため、
環境変数を変えても**古い値が埋め込まれたまま**になることがある
（実際に調査中これで誤った再現をしかけた）。
環境変数を変更したあとの検証は `--clear` を付けて行う。
