...
ユーザー単位で、**DeepSeek を実際に呼んだ回数**（`lookups.cache_hit = false`）を数える。
キャッシュヒットは上限を消費しない。
...
上限超過時は `429` / `rate_limited` を返し、`Retry-After` ヘッダも併せて返す。
...
10. save_synonym_generation（生成 + 明細 + 履歴を 1 トランザクション）
11. 200 (cached: false)
```

**手順 10 が失敗した場合**、生成自体は成功しているため、
エラーを返さず結果を返し、永続化失敗を error レベルでログに記録する。
DeepSeek のコストを既に払っている以上、ユーザーに結果を渡さないのは損失が二重になる。
（履歴に残らないことは許容する。次回同じ語を引けば再生成される。）
