...
| `rate_limited` | 429 | 制限 | LLM を呼ばない。`retryAfterSeconds` を返す | 残り時間を表示し、生成ボタンを無効化。履歴の閲覧を促す |
...
| （ネットワーク到達不可） | — | 端末側 | — | オフライン表示。生成ボタンを無効化し、キャッシュ済みの履歴を表示 |
...
| オフライン | 上部にオフラインバナー。生成ボタン無効化。履歴は端末キャッシュから表示 |
...
| `rate_limited` | 本日の生成回数の上限に達しました。保存済みの履歴はいつでも見返せます。 |
